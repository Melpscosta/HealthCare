...
export async function getAppointments(userId: string): Promise<Appointment[]> {
  try {
    ...
    const userAppointments = all.filter((a) => a.patientId === userId && a.status !== 'cancelled');
...
export async function clearCancelledAppointments(): Promise<void> {
  try {
    const stored = await AsyncStorage.getItem(APPOINTMENTS_KEY);
    if (!stored) return;
    const all: Appointment[] = JSON.parse(stored);
    ...
    if (active.length !== all.length) {
      await AsyncStorage.setItem(APPOINTMENTS_KEY, JSON.stringify(active));
    }
  } catch {
    // ignore
  }
